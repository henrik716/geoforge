import {
  DataModel, Layer, SourceConnection, SourceType,
...
// ============================================================
// Generate deploy file map (replaces zip generation)
...
export const generateDeployFiles = (
  ...
  const files: Record<string, string> = {
    ...
    'docker-compose.yml': generateDockerCompose(model, source),
    'Dockerfile': generateDockerfile(model, source),
    'pygeoapi-config.yml': generatePygeoapiConfig(model, source, lang),
    'project.qgs': generateQgisProject(model, source),
    '.env.template': generateEnvFile(source),
    '.gitignore': '.env\ndata/\n*.gpkg\n__pycache__/\n',
    'README.md': generateReadme(model, source),
    '.github/workflows/deploy.yml': generateGithubActionsWorkflow(model, source),
  };
